...
export function PdfViewer({ book }) {
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      if (document.fullscreenElement) {
        setScale(s => { const next = Math.min(s + 0.25, 3.0); db.books.update(book.id, { zoomScale: next }); return next; });
      } else {
        setScale(s => { const next = Math.max(s - 0.25, 0.5); db.books.update(book.id, { zoomScale: next }); return next; });
      }
    };
  ...
  const changePage = async (offset) => {
    ...
    if (newPage >= 1 && newPage <= pdfDoc.numPages) {
      ...
      await db.books.update(book.id, { currentPage: newPage, lastReadAt: new Date(), readingProgress: newPage / pdfDoc.numPages });
    }
  };
  ...
  if (pdfError) {
    return (
      <div className="p-8 text-center text-accent flex flex-col items-center">
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '10px', overflow: 'hidden', boxShadow: '0 4px 24px var(--shadow)', background: 'var(--surface)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px', background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: '7px', padding: '2px', boxShadow: 'inset 0 1px 3px var(--shadow)' }}>
          <button onClick={() => changePage(-pageStep)} disabled={pageNumber <= 1} style={{ ...toolbarBtn, opacity: pageNumber <= 1 ? 0.35 : 1 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0 6px' }}>
            <input type="text" defaultValue={pageNumber} key={pageNumber} onKeyDown={handleJump}
              style={{ width: '38px', textAlign: 'center', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '4px', padding: '2px 4px', fontFamily: '"Lora", serif', fontSize: '0.78rem', color: 'var(--text-primary)', outline: 'none', boxShadow: 'inset 0 1px 2px var(--shadow)' }} />
            <span style={{ fontFamily: '"Lora", serif', fontStyle: 'italic', fontSize: '0.72rem', color: 'var(--text-muted)' }}>/ {book.totalPages}</span>
          </div>
          <button onClick={() => changePage(pageStep)} disabled={!!atEnd} style={{ ...toolbarBtn, opacity: atEnd ? 0.35 : 1 }}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-primary)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)'; }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px', background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: '7px', padding: '2px', boxShadow: 'inset 0 1px 3px var(--shadow)' }}>
          <button onClick={handleZoomOut} style={toolbarBtn} title="Zoom Out"
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-primary)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)'; }}>
            <ZoomOut size={14} />
          </button>
          ...
          <button onClick={handleZoomIn} style={toolbarBtn} title="Zoom In"
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-primary)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)'; }}>
            <ZoomIn size={14} />
          </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: 'auto' }}>
          {isPortrait && (
            <button onClick={() => setDoublePageMode(m => !m)}
              style={{ ...toolbarBtn, width: 'auto', padding: '0 8px', gap: '5px', background: doublePageMode ? 'var(--accent)' : 'transparent', color: doublePageMode ? 'var(--bg-primary)' : 'var(--text-secondary)', border: `1px solid ${doublePageMode ? 'var(--accent)' : 'var(--border)'}`, borderRadius: '6px', fontSize: '0.7rem', fontFamily: '"Lora", serif', fontWeight: 600, letterSpacing: '0.02em', transition: 'background 0.18s, color 0.18s, border-color 0.18s' }}
              title={doublePageMode ? 'Switch to single page' : 'Switch to double page view'}
              onMouseEnter={e => { if (!doublePageMode) { e.currentTarget.style.background = 'var(--bg-tertiary)'; e.currentTarget.style.color = 'var(--text-primary)'; } }}
              onMouseLeave={e => { if (!doublePageMode) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)'; } }}>
              <BookOpen size={14} /><span>2-page</span>
            </button>
          )}
          <button
            onClick={() => { const el = document.getElementById('main-area') || containerRef.current; if (document.fullscreenElement) document.exitFullscreen(); else el?.requestFullscreen(); }}
            style={toolbarBtn} title="Fullscreen"
      ...
      {/* PDF Canvas Area */}
      <div ref={containerRef} className="scrollbar-vintage"
        style={{ flex: 1, overflowY: 'auto', background: 'var(--surface)', display: 'flex', justifyContent: 'center', padding: '28px 20px' }}>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: doublePageMode ? '2px' : '0', margin: '0 auto', minWidth: 'min-content' }}>

          {/* Left / single canvas */}
          <div style={{ position: 'relative', boxShadow: '0 8px 40px rgba(0,0,0,0.35)', borderRadius: doublePageMode ? '2px 0 0 2px' : '1px' }}>
            {!pdfDoc && (
              <div style={{ position: 'absolute', inset: 0, minWidth: '200px', minHeight: '280px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
